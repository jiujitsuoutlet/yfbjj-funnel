...
for (const variant of ['a', 'b']) {
  for (const [width, height] of viewports) {
    ...
    await page.screenshot({ path: new URL(`screenshots/${variant}-${width}x${height}.png`, out).pathname, fullPage: true });
...
const destinations = await browser.newContext({ userAgent, ignoreHTTPSErrors: true });
for (const [name, url] of [
  ['terms', 'https://yfbjj.autocreator.ai/legal/terms'],
  ['privacy', 'https://yfbjj.autocreator.ai/legal/privacy'],
]) {
  const destinationPage = await destinations.newPage();
  const errors = [];
  destinationPage.on('pageerror', (error) => errors.push(error.message));
  const response = await destinationPage.goto(url, { waitUntil: 'domcontentloaded' });
  results.push({
    kind: 'destination', name, url, status: response?.status(), finalUrl: destinationPage.url(),
    title: await destinationPage.title(), errors,
    pass: Boolean(response?.ok()) && errors.length === 0,
  });
  await destinationPage.close();
}
results.push({
  kind: 'destination', name: 'support', url: 'mailto:Sebastian@yogaforbjj.net',
  pass: new URL('mailto:Sebastian@yogaforbjj.net').protocol === 'mailto:',
  note: 'Verified as a syntactically valid mail destination. No email was sent.',
});
await destinations.close();
