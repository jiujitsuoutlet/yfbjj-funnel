(() => {
  ...
  const functional = new Set(['checkoutForm','offerActions','price','accessLink','legalFooter','previewBanner','announcement','backgroundImage']);
  const colors = { black:'#0a0a0a',black2:'#0d0d10',cream:'#f3f4ee',white:'#ffffff',dim:'rgba(255,255,255,.70)',red:'#dc2626' };
  const fonts = { brandSans:'Arial,Helvetica,sans-serif',systemSans:'Arial,Helvetica,sans-serif',serif:'Georgia,serif' };
  ...
  let csrf = '', pageKey = 'landing-a', revision = 0, doc, selected, selectedColumn, past = [], future = [], dirty = false, timer, generation = 0, savePromise = Promise.resolve(), media = [];
  ...
  function elementMarkup(element, lifetime = {}, certification = {}) {
    ...
    else if (element.type === 'checkoutForm') body = `<label>${esc(element.label)}</label><input disabled placeholder="${esc(element.placeholder)}"><label class="order-bump"><input type="checkbox" disabled><span><strong>${esc(element.bumpHeadline || 'Yes... add Head to Toes')} <b>+ $9</b></strong><small>${esc(element.bumpDescription || 'Three guided, 15-minute mobility sessions for inflexible grapplers.')}</small></span></label><button disabled>${esc(element.buttonText)}</button><small>${esc(element.note)}</small>`;
    ...
    const quickOffer = element.id === lifetime.afterId
      ? lifetimeQuickOfferPreview(lifetime.action)
      : element.id === certification.afterId
        ? certificationQuickOfferPreview(certification.action)
        : '';
    return `<div class="editor-element ${element.id === lifetime.heroImageId ? 'lifetime-hero-image' : ''} ${esc(preset[element.preset] || '')} ${selected === element.id ? 'selected' : ''}" data-element="${esc(element.id)}" draggable="true" style="${style(element)}">${body}</div>${quickOffer}`;
  }
  function renderCanvas() {
    ...
    const heroImageIndex=pageKey==='offer-lifetime'?all.findIndex((element)=>element.type==='image'):-1;
    const nextHeadingIndex=heroImageIndex>=0?all.findIndex((element,index)=>index>heroImageIndex&&element.type==='heading'):-1;
    const lifetimeIntro=heroImageIndex>=0?all.slice(heroImageIndex+1,nextHeadingIndex>=0?nextHeadingIndex:undefined):[];
    const lifetime=heroImageIndex>=0?{heroImageId:all[heroImageIndex].id,afterId:[...lifetimeIntro].reverse().find((element)=>element.type==='text'&&/⬇/.test(element.content||''))?.id||[...lifetimeIntro].reverse().find((element)=>element.type==='text')?.id,action:all.find((element)=>element.type==='offerActions')}:{};
    const certificationTarget=pageKey==='offer-certification'?all.find((element)=>['heading','text'].includes(element.type)&&(element.id==='heading-cfbc444a'||/grab\s+our\s+entire\s+3(?:\s|-)+part\s+instructor\s+certification\s+program\s+for\s+a\s+crazy\s+deal/i.test(element.content||''))):null;
    const certification=certificationTarget?{afterId:certificationTarget.id,action:all.find((element)=>element.type==='offerActions')}:{};
    $('canvas').innerHTML = `<div class="editor-page" style="--editor-page-bg:${colors[global.backgroundColor]};--editor-body-color:${colors[global.bodyColor]};--editor-heading-color:${colors[global.headingColor]};--editor-accent-color:${colors[global.accentColor]};--editor-page-font:${fonts[global.fontFamily]};--editor-page-width:${global.maxWidth}px;--editor-button-radius:${global.buttonRadius}px">${announcement?`<div class="announce" data-element="${announcement.id}">${esc(announcement.content)}</div>`:''}${preview?`<div class="preview-banner" data-element="${preview.id}">Preview status from server</div>`:''}${doc.sections.map((section) => {const sectionElements=section.rows.flatMap((row)=>row.columns.flatMap((column)=>column.elements)),background=sectionElements.find((element)=>element.type==='backgroundImage');return `<section class="${esc(preset[section.preset] || '')}" style="${style(section)}">${background?`<picture class="editor-background${background.flipHorizontal?' is-flipped':''}" data-element="${background.id}"><img src="${esc(background.src)}" alt="${esc(background.alt)}"></picture>`:''}<div class="editor-section-inner">${section.rows.map((row) => `<div class="editor-row" style="${style(row)}">${row.columns.map((column) => `<div class="editor-column" data-column="${column.id}" style="--editor-column:${column.width};${style(column)}">${column.elements.filter((element)=>!['announcement','backgroundImage','previewBanner','legalFooter'].includes(element.type)).map((element)=>elementMarkup(element,lifetime,certification)).join('')}</div>`).join('')}</div>`).join('')}</div></section>`;}).join('')}${legal?`<footer data-element="${legal.id}">Legal and support links from server</footer>`:''}</div>`;
    $('canvas').firstElementChild.dataset.pageKey = pageKey;
  ...
  function renderInspector() {
    ...
    for (const key of ['label','placeholder','note','buttonText','bumpHeadline','bumpDescription','acceptText','skipText','alt','attribution','title']) if (key in element) output += `<label class="field">${key}<input data-field="${key}" value="${esc(element[key])}"></label>`;
    if ('src' in element && element.type !== 'video') { const choices=[{url:'/img/guard-pass-400.webp',filename:'guard-pass-400.webp'},{url:'/img/guard-pass-800.webp',filename:'guard-pass-800.webp'},{url:'/img/guard-pass-1600.webp',filename:'guard-pass-1600.webp'},{url:'/img/lunge-wide-keep-reading-v1.webp',filename:'lunge-wide-keep-reading-v1.webp'},{url:'/img/lifetime-twist-chair-1600-v1.webp',filename:'lifetime-twist-chair-1600-v1.webp'},{url:'/img/lifetime-twist-chair-cta-v1.webp',filename:'lifetime-twist-chair-cta-v1.webp'},{url:'/img/membership-shoulders-1600-v1.webp',filename:'membership-shoulders-1600-v1.webp'},...media]; output += `<label class="field">Image<select data-field="src">${choices.map((item)=>`<option value="${esc(item.url)}" ${element.src===item.url?'selected':''}>${esc(item.filename)}</option>`).join('')}</select></label>`; }
  ...
  async function publish(){const button=$('publish');button.disabled=true;button.textContent='Publishing';try{await flushSave();if(dirty){$('save-state').textContent='Save the draft first';return;}const result=await api(`/api/admin/pages/${pageKey}/publish`,{method:'POST',body:JSON.stringify({expectedRevision:revision})});$('save-state').textContent=`Published ${result.revision}`;button.textContent='Published';}catch(error){$('save-state').textContent=error.status===409?'Changed elsewhere. Reload before publishing.':'Publish failed. Try again.';button.textContent='Try publish again';}finally{button.disabled=false;}}
  ...
  async function analytics(){const data=await api('/api/admin/analytics'),panel=$('history-panel');panel.hidden=false;panel.innerHTML=`<h2>Funnel results</h2><p>Winner gate: ${data.abDecisionRule.minimum_visitors_per_variant} visitors and ${data.abDecisionRule.minimum_purchases_per_variant} purchases per variant.</p><pre>${esc(JSON.stringify({variants:data.variants,events:data.conversion.byVariant,offers:data.conversion.byOffer,sources:data.conversion.bySource,devices:data.conversion.byDevice},null,2))}</pre>`;}
  async function upload(file){if(!file)return;if(file.size>768000){$('media-state').textContent='Image must be under 750 KB.';return;}$('media-state').textContent='Uploading';const data=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',')[1]);reader.onerror=reject;reader.readAsDataURL(file);});const result=await api('/api/admin/media',{method:'POST',body:JSON.stringify({filename:file.name,contentType:file.type,data})});media.unshift(result.media);$('media-state').textContent='Uploaded. Select an image block to use it.';renderInspector();}
  ...
  $('undo').onclick=()=>{if(!past.length)return;future=[structuredClone(doc),...future];doc=past.pop();markChanged();};$('redo').onclick=()=>{if(!future.length)return;past.push(structuredClone(doc));doc=future.shift();markChanged();};$('save').onclick=()=>save();$('publish').onclick=publish;$('versions').onclick=versions;$('analytics').onclick=analytics;$('add-section').onclick=addSection;$('media-upload').onchange=(event)=>upload(event.target.files[0]).catch(()=>$('media-state').textContent='Upload failed');
  ...
  $('add-tools').innerHTML=['heading','text','list','quote','video','image','divider','spacer'].map((type)=>`<button data-add="${type}">${type}</button>`).join('');document.querySelectorAll('[data-add]').forEach((button)=>button.onclick=()=>{remember();const type=button.dataset.add,node={id:id(type),type};if(type==='heading')Object.assign(node,{level:2,content:'New heading'});if(type==='text')node.content='New text';if(type==='list')node.items=['New item'];if(type==='quote')Object.assign(node,{content:'Member quote',attribution:'Yoga for BJJ member'});if(type==='video')Object.assign(node,{src:'https://iframe.mediadelivery.net/embed/215008/ad1f2932-955f-4abf-85d0-01c6a065a289?autoplay=false&loop=false&muted=false&preload=true&responsive=true&controls=true',title:'Yoga for BJJ video'});if(type==='image')Object.assign(node,{src:'/img/guard-pass-800.webp',alt:'Yoga for BJJ'});if(type==='spacer')node.size=24;const column=findColumn(selectedColumn)||doc.sections[0].rows[0].columns[0];column.elements.push(node);selected=node.id;selectedColumn=column.id;markChanged();});
  ...
  api('/api/admin/session').then((session)=>{csrf=session.csrf;return api('/api/admin/media');}).then((result)=>{media=result.media;return load();}).catch(()=>location.href='/admin/login');
})();
